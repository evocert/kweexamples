...
WGET_MAX_VISIT=1024
WGET_MAX_DEPTH=4
curl -X GET "$URL?cmd=wget&url="$WGET_TGT"&recursive="$WGET_RECURSIVE"&debug="$WGET_DEBUG"&maxvisit="$WGET_MAX_VISIT"&maxdepth="$WGET_MAX_DEPTH"&output="$WGET_OUTPUT"" --output -
