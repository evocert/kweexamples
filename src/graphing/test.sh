...
#--------------------------------------------------------------------------------
curl -X POST -H "Content-Type: application/json" --data '{"cmd":"tree","outfmt":"'$OUTFMT'"}' "$URL" --output -
...
#curl -X POST -H "Content-Type: text/plain" --data '' "$URL?decrypt=true" --output -
#curl -X POST -H "Content-Type: text/plain" --data 'U2FsdGVkX1/GTxrtkqofHHev1SpK1BFWIUhmT+k2c3mMJ+ak4rnf7ezDpwbGPJ/rZDoA7vZ0Lp3uS0YWsRoTVKaRgL5WW+fwXP8cx0c/HZU948DGQHgnom3B/uOZ08VFj6/BmCUNda1vCMYwTEeQXyioxaVGKg4lbBEaFICF2axRPK2lLjMO/2v0T8bbSryTKDOY1Ckkxfa+SXZTZLfYKCf3NC4NZU3/OBDecejigY8=' "$URL?decrypt=true" --output -
...
#echo pipe test
#curl -X POST -H "Content-Type: application/json" --data '{"cmd":[{"cmd":"add","a":4,"b":2},{"cmd":"log"},{"cmd":"neg"},{"cmd":"log"},{"cmd":"exp"},{"cmd":"log"},{"cmd":"abs"}],"encrypt":false,"pipe":true}' "$URL" --output -
#curl -X POST -H "Content-Type: application/json" --data '{"cmd":[{"cmd":"add","a":4,"b":2},{"cmd":"log"},{"cmd":"neg"},{"cmd":"log"},{"cmd":"exp"},{"cmd":"log"},{"cmd":"abs"}],"encrypt":false,"pipe":true,"encrypt":true}' "$URL" --output -
#
